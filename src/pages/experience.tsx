...
export default function Experiences() {
  return (
    ...
    >
      ...
      {/* Top-left purple orb */}
      <motion.div
        className="absolute top-[10%] left-[-12%] w-72 h-72 rounded-full bg-[#a855f7] opacity-45 blur-[120px] z-0"
        animate={{ y: [0, 20, 0], x: [0, -30, 0] }}
        transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Top-right indigo orb */}
      <motion.div
        className="absolute top-[15%] right-[-12%] w-64 h-64 rounded-full bg-[#6366f1] opacity-40 blur-[100px] z-0"
        animate={{ y: [0, -15, 0], x: [0, 25, 0] }}
        transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Bottom-left red orb */}
      <motion.div
        className="absolute bottom-[15%] left-[-12%] w-80 h-80 rounded-full bg-[#f43f5e] opacity-30 blur-[140px] z-0"
        animate={{ scale: [1, 1.1, 1] }}
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Bottom-right purple orb */}
      <motion.div
        className="absolute bottom-[20%] right-[-12%] w-56 h-56 rounded-full bg-[#a855f7] opacity-35 blur-[110px] z-0"
        animate={{ scale: [1, 1.05, 1], x: [0, 10, 0] }}
        transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Mid-left teal orb */}
      <motion.div
        className="absolute top-[50%] left-[5%] w-56 h-56 rounded-full bg-[#14b8a6] opacity-30 blur-[100px] z-0"
        animate={{ y: [0, -20, 0], x: [0, 15, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Mid-right amber orb */}
      <motion.div
        className="absolute bottom-[50%] right-[5%] w-64 h-64 rounded-full bg-[#facc15] opacity-25 blur-[120px] z-0"
        animate={{ y: [0, 20, 0], x: [0, -15, 0] }}
        transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
      />
      {/* Center subtle indigo orb */}
      <motion.div
        className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-96 h-96 rounded-full bg-[#6366f1] opacity-10 blur-[180px] z-0"
      ...
      <div className="relative z-10 max-w-3xl w-full">
        ...
        <div className="relative before:absolute before:inset-y-0 before:left-6 before:w-[2px] before:bg-white/10 pl-10 space-y-12">
          {experiences.map((exp, i) => (
            <motion.div
              ...
              className="relative group before:absolute before:top-6 before:left-[-1.35rem] before:w-4 before:h-4 before:rounded-full before:bg-black before:border-2 before:border-accent shadow before:z-10"
            >
              <div className="bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-md shadow-lg transition-all duration-300 ease-in-out group hover:border-accent hover:bg-white/10 hover:scale-[1.015]">
