...
export default function Skills() {
  return (
    ...
    >
      ...
      <motion.div
        className="absolute top-[12%] left-[8%] w-64 h-64 rounded-full bg-[#818cf8] opacity-40 blur-[120px] z-0"
        animate={{ y: [0, 30, 0], x: [0, -25, 0] }}
        transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
      />

      <motion.div
        className="absolute bottom-[-6%] right-[10%] w-72 h-72 rounded-full bg-[#d946ef] opacity-35 blur-[140px] z-0"
        animate={{ y: [0, -30, 0], x: [0, 20, 0] }}
        transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
      />

      <motion.div
        className="absolute bottom-[-14%] left-[32%] w-[500px] h-[500px] rounded-full bg-[#f43f5e] opacity-25 blur-[180px] z-0"
      ...
      >
        {/* Title */}
        <h2 className="text-4xl md:text-5xl font-bold text-center mb-14">Skills & Technologies</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 w-full">
          {skills.map((section, i) => (
            <motion.div
              ...
              className="rounded-2xl p-6 bg-white/5 border border-white/10 backdrop-blur-lg hover:shadow-2xl hover:border-accent transition"
            >
