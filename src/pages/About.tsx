...
export default function About() {
  return (
    <section className=" will-change relative min-h-screen px-6 py-24 flex flex-col lg:flex-row items-center justify-center gap-16 overflow-hidden bg-black text-white transition-colors duration-300">

      {/* ✅ Animated Background Orbs */}
      <motion.div
        className="will-change absolute top-[8%] left-[6%] w-56 h-56 rounded-full bg-indigo-500 opacity-40 blur-[100px] z-0"
        animate={{ y: [0, 25, 0], x: [0, -25, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
      />

      <motion.div
        className="will-change absolute top-[50%] right-[0%] w-72 h-72 rounded-full bg-sky-500 opacity-35 blur-[120px] z-0"
        animate={{ y: [0, -30, 0], x: [0, 30, 0] }}
        transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}
      />

      <motion.div
        className="will-change absolute bottom-[-12%] left-[28%] w-[480px] h-[480px] rounded-full bg-pink-500 opacity-30 blur-[160px] z-0"
      ...
      >
        <img
          src={profile}
          alt="Danish"
          className="will-change w-60 h-60 lg:w-72 lg:h-72 rounded-full object-cover shadow-2xl border-4 border-white dark:border-white/20"
        />
      </motion.div>
      ...
      >
        <h2 className="will-change text-4xl font-bold mb-4">About Me</h2>
